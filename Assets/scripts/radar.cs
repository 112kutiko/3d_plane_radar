...
public class radar : MonoBehaviour
{
	...
	public void change_cam(){ 
		...
		}else{ for(int c = 0; c < pl_List.Count; c++){
					if (pl_List[c].Icao == _id){fog.SetActive(false);
	...
	public void first_time(List<IdList> a){
		foreach (IdList x in a){plane_spawner(x, "first time");	}	
	}
	public void data_update(){ 
		if (tempory_plane.Count != 0){
			foreach (IdList x in tempory_plane){
				if (GameObject.Find(x.Icao) != null){plane_update(x);
				}else{plane_spawner(x, "secon time");}
			}
		}
	}
	public void check_or_exsist(){
		List<string> _on = new List<string>(),_off = new List<string>();
		for (int u = 0; u < tempory_plane.Count; u++){for (int a = 0; a < pl_List.Count; a++){
				if (tempory_plane[u].Id == pl_List[a].Id)	{_on.Add(pl_List[a].Id);}
			}
		}
		for (int a = 0; a < pl_List.Count; a++){_off.Add(pl_List[a].Id);}
		List<string> _d = _off.Except(_on).ToList();
			for (int s = 0; s < pl_List.Count; s++){
				for(int y = 0; y < _d.Count; y++){
                if (pl_List[s].Id == _d[y]){dell_checked_plane(pl_List, pl_List[s]);}
            }}		
		tempory_plane.Clear();
		}
	...
	public void text_reload(){
        ...
        if (pl_List.Count != 0){
		for (int i= 0;i<pl_List.Count;i++){
			if (tmp_ac ==""){ tmp_ac = " Icao " + pl_List[i].Icao + " call " + pl_List[i].Call + " \n";
			}else{ tmp_ac = tmp_ac + " Icao " + pl_List[i].Icao + " call " + pl_List[i].Call + " \n";}
		}
	  }	text_box_update(tmp_ac);
	}
	public GameObject this_plane(string i){
		GameObject o= plane_pl[0];
		if (i == "true"){o = plane_pl[1];}
		return o;
    }
	...
	public void dell_all() {
		...
		for(int o=0;o< pl_List.Count; o++)	{
				if(pl_List[o].Icao== _now_plane){pl_List[o].plane.GetComponent<plane_cam_hold>().cam_back();}
